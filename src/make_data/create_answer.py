...
total.to_csv('rsc/sql/answer.csv')

inner = pd.merge(answer_button, total['url'], left_on='button_url', right_on='url', how='inner').drop('url', axis=1)
outer = pd.merge(answer_button, total['url'], left_on='button_url', right_on='url', how='outer')
left = outer[outer['url'].isna()].drop('url', axis=1)
outside = left[left['button_name'].str.contains('사이트로 이동')]
print(outside)
outside['button_name'] = '사이트로 이동'
print(outside)
inner = pd.concat([inner, outside])
inner.to_csv('rsc/sql/answer_button.csv')